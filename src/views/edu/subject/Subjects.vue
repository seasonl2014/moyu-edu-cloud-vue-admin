  ...
  export default {
    data() {
      return {
        ...
        editRuleForm:[],
        addRuleForm: { parentId: "0", name: "", remark: "", sort: "" }, //添加表单
        addRules: {
          name: [
    ...
    methods: {
        ...
        this.$refs.editRuleFormRef.validate(async valid => {
          if (!valid) {
            ...
            const {data: res} = await this.$http.put(
                    "eduservice/subject/update/" + this.editRuleForm.id,
                    this.editRuleForm
            );
            if (res.code === 200) {
              this.$notify({
      ...
      //修改
      async edit(id) {
        const { data: res } = await this.$http.get("eduservice/subject/edit/" + id);
        if (res.code === 200) {
      ...
      selectParentChange() {
        ...
        if (len > 0) {
          this.addRuleForm.parentId = this.pKeys[len - 1];
        }else{
      ...
      async getParentCategoryList() {
        const { data: res } = await this.$http.get(
                "eduservice/subject/getParentEduSubjectTreeNode"
        );
      ...
      async add() {
        this.$refs.addRuleFormRef.validate(async valid => {
          if (!valid) {
            ...
            this.btnLoading=true;
            this.btnDisabled=true;
            if(this.addRuleForm.parentId == null){
              this.addRuleForm.parentId=0;
            }
            const { data: res } = await this.$http.post(
                    "eduservice/subject/add",
                    this.addRuleForm
            );
            if (res.code === 200) {
